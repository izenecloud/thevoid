...
void add_file_frontend(logger_base &log, const std::string &file, log_level level)
{
	...
	auto sink = blackhole::aux::util::make_unique<blackhole::sink::files_t<>>(blackhole::sink::files_t<>::config_type(file));
	auto frontend = blackhole::aux::util::make_unique<blackhole::frontend_t<blackhole::formatter::string_t, blackhole::sink::files_t<>>>(std::move(formatter), std::move(sink));
...
} } } } // namespace izenecloud::swarm::utils::logger
